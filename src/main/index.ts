...
import { DEFAULT_VISION_CONFIG, DEEPSEEK_VISION_BASE, resolveVisionRuntime, resolveVisionSettings, visionTitle, type VisionConfig, type VisionProvider } from "../shared/vision-api";
...
function registerIpc(): void {
  ...
  ipcMain.handle("workspace:reveal", async (_event, relativePath: string, workspacePath?: string) => {
    shell.showItemInFolder(await resolveInWorkspace(typeof relativePath === "string" && relativePath.trim() ? relativePath : ".", workspacePath));
  });
  ipcMain.handle("workspace:restore", async (_event, files: unknown, workspacePath?: string) => {
  ...
  ipcMain.handle("workspace:list", async (_event, workspacePath?: string) => {
    const root = path.resolve(typeof workspacePath === "string" && workspacePath ? workspacePath : activeAgentCwd ?? "");
    if (!root) return [];
    const allowed = path.resolve(activeAgentCwd ?? "") === root
  ...
  ipcMain.handle("vision:config", async () => {
    ...
    const chatKey = profiles?.kind === "deepseek" ? profiles.deepseek.apiKey.trim() : "";
  ...
  ipcMain.handle("vision:save-config", async (_event, next: {
    ...
    const config: VisionConfig = settings.provider === "deepseek"
      ? await materializeDeepSeekVision(apiKey)
      : { ...settings, apiKey };
    await fsp.writeFile(visionConfigPath(), `${JSON.stringify(config, null, 2)}\n`, { mode: 0o600 });
  });
  ipcMain.handle("vision:stage", async (_event, images: string[]) => {
    const refs = Array.isArray(images) ? images.filter((item) => typeof item === "string" && item).slice(0, 4) : [];
    if (refs.length === 0) throw new Error("先上传至少一张图片");
    const dir = visionUploadsDir();
    await fsp.mkdir(dir, { recursive: true, mode: 0o700 });
    const stamp = Date.now();
    return Promise.all(refs.map(async (item, index) => {
  ...
  ipcMain.handle("sessions:list", async (_event, cwd?: string) => {
    ...
    return threads.map((thread): SessionSummary => ({
  ...
  ipcMain.handle("auth:status", async (): Promise<ProviderStatus[]> => {
    ...
    return SUPPORTED_PROVIDER_IDS.filter((id) => id !== "openai-codex").map((id) => {
      ...
      return {
        ...
        ...(hasStore ? { source: "stored" as const } : environment ? { source: "environment" as const } : {}),
        defaultModel: stored?.providerId === id && stored.modelId ? stored.modelId : "",
        ...(id === "deepseek" && deepseekUrl ? { baseUrl: deepseekUrl } : {}),
        ...(stored?.providerId === id ? { preferred: true } : {}),
      };
    });
  });
  ipcMain.handle("auth:read-api-key", async (_event, provider: ApiKeyProviderId) => {
    const stored = await (await createTetherCredentialStore()).read(provider);
    if (stored && stored.type === "api_key" && typeof stored.key === "string") return stored.key;
    const envName = providerEnvironmentKey(provider);
    return envName ? process.env[envName]?.trim() ?? "" : "";
  });
  ipcMain.handle("auth:save-api-key", async (_event, provider: ApiKeyProviderId, key: string, baseUrl?: string, model?: string) => {
    if (typeof key === "string" && key.trim()) await saveProviderApiKey(provider, key.trim());
  ...
  ipcMain.handle("auth:list-models", async (_event, baseUrl: string, apiKey: string) => {
    if (typeof baseUrl !== "string" || typeof apiKey !== "string") throw new Error("先填写 API URL 和 Key");
  ...
  ipcMain.handle("agent:start", async (_event, options: AgentStartOptions) => {
    ...
    const { resume: _resume, sandbox: requestedSandbox, storagePath, ...startOptions } = options;
    let sessionPath = startOptions.sessionPath;
    if (sessionPath) {
      sessionPath = await ensureSessionRuntimeLink(sessionPath, storagePath || sessionPath);
    }
  ...
  ipcMain.handle("agent:command", async (_event, type: string, data?: Record<string, unknown>) => {
    ...
    if (type === "new_session" || type === "get_state" || type === "get_session_stats") {
...
async function saveDefaultModel(providerId: string, modelId: string): Promise<void> {
  const settingsPath = path.join(getTetherHome(), "settings.json");
  let settings: Record<string, unknown> = {};
  try {
    settings = JSON.parse(await fsp.readFile(settingsPath, "utf8")) as Record<string, unknown>;
  } catch {
    /* first write */
  }
...
const recentWorkspaces = {
  async list(): Promise<WorkspaceItem[]> {
    try {
      const parsed = JSON.parse(await fsp.readFile(recentFile, "utf8")) as unknown;
  ...
  async touch(workspacePath: string): Promise<string> {
    ...
    const next = [
      { path: resolved, name: path.basename(resolved) || resolved, lastOpenedAt: new Date().toISOString() },
...
async function loadChatProfiles(): Promise<ChatProfiles> {
  try {
    const parsed = parseChatProfiles(JSON.parse(await fsp.readFile(chatProfilesPath(), "utf8")));
    if (parsed) return parsed;
  } catch {
    /* migrate from the single stored slot */
  }
  const stored = await (await createTetherCredentialStore()).read("deepseek");
  const apiKey = stored && stored.type === "api_key" && typeof stored.key === "string" ? stored.key : "";
...
async function saveChatProfiles(next: ChatProfiles): Promise<void> {
  ...
  await fsp.mkdir(path.dirname(chatProfilesPath()), { recursive: true, mode: 0o700 });
  await fsp.writeFile(chatProfilesPath(), `${JSON.stringify(merged, null, 2)}\n`, { mode: 0o600 });
...
async function loadVisionConfig(): Promise<VisionConfig> {
  try {
    const raw = JSON.parse(await fsp.readFile(visionConfigPath(), "utf8")) as Partial<VisionConfig>;
    const settings = resolveVisionSettings(raw);
    const base: VisionConfig = {
      ...settings,
      apiKey: typeof raw.apiKey === "string" ? raw.apiKey.trim() : "",
    };
    if (base.provider === "deepseek") return materializeDeepSeekVision(base.apiKey);
    return base;
  } catch {
...
async function materializeDeepSeekVision(fallbackKey = ""): Promise<VisionConfig> {
  const store = await createTetherCredentialStore();
  try {
    const profiles = await loadChatProfiles().catch(() => undefined);
    const stored = await store.read("deepseek");
    const storedKey = stored && stored.type === "api_key" && typeof stored.key === "string"
      ? stored.key.trim()
      : "";
    ...
    const chatKey = profiles?.kind === "deepseek" ? (profiles.deepseek.apiKey.trim() || storedKey) : "";
    const key = fallbackKey.trim() || chatKey || process.env.DEEPSEEK_API_KEY?.trim() || "";
...
async function syncDeepSeekVisionConfig(): Promise<void> {
  ...
  await fsp.writeFile(visionConfigPath(), `${JSON.stringify(next, null, 2)}\n`, { mode: 0o600 });
}

async function resolveInWorkspace(relativePath: string, workspacePath?: string): Promise<string> {
  const candidate = typeof workspacePath === "string" && workspacePath.trim() ? workspacePath : activeAgentCwd;
...
function sessionFileOf(snapshot: AgentSnapshot): string | undefined {
  return sessionFileFromUnknown(snapshot.stats) ?? sessionFileFromUnknown(snapshot.state);
}
...
function watchWorkspace(root: string): void {
  ...
  try {
    workspaceWatcher = fs.watch(root, { persistent: false, recursive: true }, (_event, filename) => {
...
function skipWatch(filename: string | null): boolean {
  ...
  return filename.replaceAll("\\", "/").split("/").some((part) => SKIP_DIRS.has(part) || (part.startsWith(".") && part !== ".agents"));
}

// ponytail: dirs always complete; files capped globally + per folder so DFS doesn't starve later siblings.
async function listWorkspaceFiles(root: string, fileLimit = 8000, perDirLimit = 200): Promise<string[]> {
  const dirs: string[] = [];
  const files: string[] = [];
  async function walk(dir: string): Promise<void> {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        ...
        dirs.push(`${path.relative(root, path.join(dir, entry.name)).replaceAll("\\", "/")}/`);
