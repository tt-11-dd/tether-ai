...
let pendingUpdateNotice: { version: string } | undefined;
...
const DOWNLOAD_SITE = "https://tether-code.xyz/";

async function checkForUpdates(manual = false): Promise<void> {
  if (!manual && updateCheckStarted) return;
  ...
  try {
    ...
    if (!manual) {
      pendingUpdateNotice = { version: update.version };
      if (!window.isDestroyed()) window.webContents.send("app:update-available", pendingUpdateNotice);
      return;
    }
    ...
    if (result.response === 0) await shell.openExternal(DOWNLOAD_SITE);
  } catch {
    ...
    const result = await dialog.showMessageBox(mainWindow, {
      ...
      detail: t(appLocale, "update.failedDetail"),
      buttons: [t(appLocale, "update.download"), t(appLocale, "update.ok")],
      defaultId: 0,
      cancelId: 1,
      noLink: true,
    });
...
function registerIpc(): void {
  ...
  ipcMain.handle("app:update-notice", () => pendingUpdateNotice);
